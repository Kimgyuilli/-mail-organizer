...
async def test_list_gmail_messages_success(
    ...
    response = await client.get(f"/api/gmail/messages?user_id={sample_user.id}")
...
async def test_list_gmail_messages_pagination(
    ...
    response = await client.get(
        f"/api/gmail/messages?user_id={sample_user.id}&offset=0&limit=1"
    )
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 2
...
async def test_get_gmail_message_success(
    ...
    response = await client.get(
        f"/api/gmail/messages/{mail_id}?user_id={sample_user.id}"
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_get_gmail_message_not_found(client: AsyncClient, sample_user):
    ...
    response = await client.get(
        f"/api/gmail/messages/999?user_id={sample_user.id}"
    )
...
async def test_get_gmail_message_user_not_found(client: AsyncClient):
    """GET /api/gmail/messages should return 404 when user doesn't exist."""
    response = await client.get("/api/gmail/messages?user_id=999")
    assert response.status_code == 404
    data = response.json()
